...
These three runs in random effective mode and then interest mode generally seem to confirm that maximising goal *slice size* per parent *slice size* does slightly increase the *model* growth rate. We can see that the average *likelihood* and hit *likelihood* are fairly constant across both modes, with hit *likelihood* considerably higher than average *likelihood* at around 0.89 versus around 0.38. The new hit *likelihood* measure is higher than it was with the old *likelihood* measure, which was around 0.63. In interest mode the marginal success rate of around 9% is producing more decidable *slices*, higher hits, lower hit length and hence higher *fuds* per *size* per threshold.
